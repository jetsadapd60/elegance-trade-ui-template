...
export class TokenMarketingService {
  ...
  initToken() {
    this._accessToken   = this.storage.getItem<string>(MarketingStorage.MARKETING_ACCESS_TOKEN);
    this._refreshToken  = this.storage.getItem<string>(MarketingStorage.MARKETING_ACCESS_TOKEN);
  ...
  setAccessToken(access_token: string): void {
    if (access_token) {
      this.storage.setItem<string>(MarketingStorage.MARKETING_ACCESS_TOKEN, access_token);
      this.initToken();
    }
  }
  ...
  setRefreshToken(refresh_token: string): void {
    if (refresh_token) {
      this.storage.setItem<string>(MarketingStorage.MARKETING_ACCESS_TOKEN, refresh_token);
      this.initToken();
    }
  }
